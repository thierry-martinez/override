...
other compiled interface files.
This library generalizes [`ppx_import`][1] by allowing a whole
module to be imported with all its types, possibly with annotations.
In particular, importing a whole module can be convenient to apply
[`ppx_deriving`][2] to a large family of mutually inductive data types
(see examples below).

[1] https://github.com/ocaml-ppx/ppx_import
[2] https://github.com/ocaml-ppx/ppx_deriving
...
You may use `override` in the toplevel with the help of `findlib`[7].

[7] http://projects.camlcity.org/projects/findlib.html
...
mechanization of Gabriel Scherer's post about [Overriding submodules][3]
on Gagallium blog
(it is worth noticing that, thanks to [GPR#1892][10], module overriding is
...
[3] http://gallium.inria.fr/blog/overriding-submodules/
[10] https://github.com/ocaml/ocaml/pull/1892

Let's begin with the same example as in the blog post, with the
following module defined in [`toto.ml`][4].

[4] https://gitlab.inria.fr/tmartine/override/blob/master/examples/gagallium/toto.ml
...
In another file, say [`gagallium.ml`][5], we can override the module
`Toto` to change the definition of `y` for a `string`, for example.

[5] https://gitlab.inria.fr/tmartine/override/blob/master/examples/gagallium/gagallium.ml
...
[`gagallium.mli`][6], in the same lines as above.

[6] https://gitlab.inria.fr/tmartine/override/blob/master/examples/gagallium/gagallium.mli
...
to modules generated by `Map.Make` ([`map_diff.ml`][8]).

[8] https://gitlab.inria.fr/tmartine/override/blob/master/examples/map_diff/map_diff.ml
...
[9] https://gitlab.inria.fr/tmartine/override/blob/master/examples/longident_show/longident_show.ml
...
and all its dependencies as below (`parsetree_show.ml`[11]).
...
[11] https://gitlab.inria.fr/tmartine/override/blob/master/examples/parsetree_show/parsetree_show.ml
...
([`package_type.ml`][12]), relying on rewriting rules to add annotations
to the occurrences of `Longident.t` and `Asttypes.loc`.

[12] https://gitlab.inria.fr/tmartine/override/blob/master/examples/package_type/package_type.ml
...
described in the comments of [ast/ast.ml][13] from `ppxlib`.

[13] https://github.com/ocaml-ppx/ppxlib/blob/master/ast/ast.ml.
